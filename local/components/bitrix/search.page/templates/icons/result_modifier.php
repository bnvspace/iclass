            ...
            case "iblock":
                if (substr($arItem["ITEM_ID"], 0, 1) === "G") {
                    ...
                    if ( ! $file && $SECTION_ID) {
                        ...
                        if ($arSection = $rsSection->Fetch()) {
                            if (strlen($arSection["CODE"]) &&
                                file_exists($abs_path . "iblock_section_" . strtolower($arSection["CODE"]) . ".gif")
                            ) {
                                $file = "iblock_section_" . strtolower($arSection["CODE"]) . ".gif";
                            } elseif (file_exists($abs_path . "iblock_section_" . strtolower($arSection["ID"]) .
                                                  ".gif")) {
                                $file = "iblock_section_" . strtolower($arSection["ID"]) . ".gif";
                            } elseif (strlen($arSection["XML_ID"]) &&
                                      file_exists($abs_path . "iblock_section_" . strtolower($arSection["XML_ID"]) .
                                                  ".gif")
                            ) {
                    ...
                    if ( ! $file && preg_match("/\\.([a-z]+?)$/i", $arItem["TITLE"], $match)) {
                        if (file_exists($abs_path . "iblock_type_" .
                                        strtolower($arIBlocks[$arItem["PARAM2"]]["IBLOCK_TYPE_ID"]) . "_" . $match[1] .
                                        ".gif")) {
                            $file = "iblock_type_" . strtolower($arIBlocks[$arItem["PARAM2"]]["IBLOCK_TYPE_ID"]) . "_" .
                                    $match[1] . ".gif";
                        }
                    }
                    //We still failed to find icon? Try iblock itself
                    if ( ! $file) {
                        if (strlen($arIBlocks[$arItem["PARAM2"]]["CODE"]) && file_exists($abs_path . "iblock_iblock_" .
                                                                                         strtolower($arIBlocks[$arItem["PARAM2"]]["CODE"]) .
                                                                                         ".gif")
                        ) {
                            $file = "iblock_iblock_" . strtolower($arIBlocks[$arItem["PARAM2"]]["CODE"]) . ".gif";
                        } elseif (file_exists($abs_path . "iblock_iblock_" .
                                              strtolower($arIBlocks[$arItem["PARAM2"]]["ID"]) . ".gif")) {
                            $file = "iblock_iblock_" . strtolower($arIBlocks[$arItem["PARAM2"]]["ID"]) . ".gif";
                        } elseif (strlen($arIBlocks[$arItem["PARAM2"]]["XML_ID"]) &&
                                  file_exists($abs_path . "iblock_iblock_" .
                                              strtolower($arIBlocks[$arItem["PARAM2"]]["XML_ID"]) . ".gif")
                        ) {
                            $file = "iblock_iblock_" . strtolower($arIBlocks[$arItem["PARAM2"]]["XML_ID"]) . ".gif";
                        } elseif (file_exists($abs_path . "iblock_type_" .
                                              strtolower($arIBlocks[$arItem["PARAM2"]]["IBLOCK_TYPE_ID"]) . ".gif")) {
                            $file = "iblock_type_" . strtolower($arIBlocks[$arItem["PARAM2"]]["IBLOCK_TYPE_ID"]) .
                                    ".gif";
                        }
                    }
    ...
    if (CModule::IncludeModule('intranet')) {
        ...
        foreach ($arResult["SEARCH"] as $i => $arItem) {
            if ($arItem["MODULE_ID"] === "intranet" && substr($arItem["ITEM_ID"], 0, 1) === "U") {
                $rsUser = CUser::GetList(($by = ''), ($ord = ''),
                    array("ID_EQUAL_EXACT" => substr($arItem["ITEM_ID"], 1),), array('SELECT' => array('UF_*')));
                $arUser = $rsUser->Fetch();
                if ($arUser) {
                    ...
                    if ($arUser['PERSONAL_BIRTHDAY']) {
                        $arBirthDate           = ParseDateTime($arUser['PERSONAL_BIRTHDAY'],
                            CSite::GetDateFormat('SHORT'));
                        $arUser['IS_BIRTHDAY'] = (intval($arBirthDate['MM']) == date('n')) &&
                                                 (intval($arBirthDate['DD']) == date('j'));
                    }
        ...
        if ( ! empty($arRes)) {
            foreach ($arRes as $key => $val) {
                $arResult['USER_PROP'][$val["FIELD_NAME"]] = (strLen($val["EDIT_FORM_LABEL"]) >
                                                              0 ? $val["EDIT_FORM_LABEL"] : $val["FIELD_NAME"]);
            }
        }
    }
?>