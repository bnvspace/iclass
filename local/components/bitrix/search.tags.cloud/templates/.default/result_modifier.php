...
//$arParams["ANGULARITY"] = floatval($arParams["ANGULARITY"]) > 0 ? $arParams["ANGULARITY"] : 0.7;
    ...
    if (strLen($arParams["COLOR_OLD"]) == 6 && hexdec($arParams["COLOR_OLD"]) > 0) {
        $arParams["COLOR_OLD"] = array(
            hexdec(substr($arParams["COLOR_OLD"], 0, 2)),
            hexdec(substr($arParams["COLOR_OLD"], 2, 2)),
            hexdec(substr($arParams["COLOR_OLD"], 4, 2))
        );
    } else {
        $arParams["COLOR_OLD"] = array(200, 200, 200);
    }
    if (strLen($arParams["COLOR_NEW"]) == 6 && hexdec($arParams["COLOR_NEW"]) > 0) {
        $arParams["COLOR_NEW"] = array(
            hexdec(substr($arParams["COLOR_NEW"], 0, 2)),
            hexdec(substr($arParams["COLOR_NEW"], 2, 2)),
            hexdec(substr($arParams["COLOR_NEW"], 4, 2))
        );
    } else {
        ...
        foreach ($arResult["SEARCH"] as $key => $res) {
            if ($arResult["CNT_ALL"] != 0) {
                ...
                $font_size = ($cnt / $arResult["CNT_ALL"]) *
                             (($arParams["FONT_RANGE"] * $arParams["ANGULARITY"]) + 1) *
                             ($arParams["FONT_RANGE"] * $arParams["ANGULARITY"]) / 2 +
                             pow(($cnt - $arResult["CNT_MIN"]) / max(1, $arResult["CNT_MAX"] - $arResult["CNT_MIN"]),
                                 0.8) *
            ...
            foreach ($arParams["COLOR_OLD"] as $k => $old) {
                ...
                } elseif ($arParams["COLOR_TYPE"] == "REAL" &&
                          (intVal($arResult["TIME_MAX"] - $arResult["TIME_MIN"]) > 0)
                ) {
