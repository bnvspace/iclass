<? if ( ! defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) {
    ...
    if ( ! function_exists("__sortName")) {
        function __sortName($arParam = [], $next)
        {
            if ($arParam["NAME_HTML"] != $next["NAME_HTML"]) {
                $arSort = array($arParam["NAME_HTML"], $next["NAME_HTML"]);
                sort($arSort);
                if ($arSort[0] != $arParam["NAME_HTML"]) {
                    return 1;
                }
            } elseif (intVal($arParam["CNT"]) < intVal($next["CNT"])) {
                return 1;
            }
    ...
    if ( ! empty($arParams["URL_SEARCH"])) {
        $arResult["~URL"] = CComponentEngine::MakePathFromTemplate($arParams["URL_SEARCH"],
    ...
    if ($this->StartResultCache(false, array($USER->GetGroups()))) {
        ...
        if ($arParams["PERIOD"] > 0) {
            $arFilter["DATE_CHANGE"] = Date(CDatabase::DateFormatToPHP(CLang::GetDateFormat("FULL", LANG)),
                time() - ($arParams["PERIOD"] * 24 * 3600) + CTimeZone::GetOffset());
        }
                ...
                if ( ! empty($tags)) {
                    ...
                    $result                   = array(
                        ...
                        "TAG_PATH"    => $APPLICATION->GetCurPageParam("tags=" . urlencode(implode(",", $url)),
                            array("tags")),
                        "TAG_WITHOUT" => $APPLICATION->GetCurPageParam((count($url_without) > 0 ? "tags=" .
                                                                                                  urlencode(implode(",",
                                                                                                      $url_without)) : ""),
                            array("tags")),
                    );
